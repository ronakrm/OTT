...
def ottVAEMNIST(niters, batch_size, lr, myTTrank, trainX):
    ...
    # Q_c1 = tf.get_variable('Qc1', shape=[1])
    ...
    def Q(X):
        ...
        o1 = tf.transpose(Q_W1.mult(tf.transpose(X))) + Q_b1
        h = tf.nn.relu(o1)
    ...
    EucGnVs = opt.compute_gradients(loss, [Q_b1, Q_b2_mu, Q_b2_sigma, P_b1, P_b2, P_W1, P_W2, Q_W2_mu, Q_W2_sigma])
    # EucGnVs = opt.compute_gradients(loss)
    ...
    for it in range(niters):        
        ...
        # print(sess.run(EucGnVs[0][0], feed_dict={X: X_mb}))
        # print(sess.run(EucGnVs[0][1], feed_dict={X: X_mb}))
        if sess.run(tf.reduce_any(tf.is_nan(AottGnVs[0][0])), feed_dict={X: X_mb}):
            print(sess.run(AottGnVs[0][1], feed_dict={X: X_mb} ))
